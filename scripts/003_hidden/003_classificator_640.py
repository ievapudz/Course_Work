...
optimizer = torch.optim.Adam(mlp_2_640.parameters(), lr=1e-4)

for epoch in range(0, NUM_OF_EPOCHS):
    # Print epoch
    print(f'Starting epoch {epoch+1}')
    
    train_epoch(mlp_2_640, trainloader, loss_function, optimizer, BATCH_SIZE, 
                EPOCH_BATCH_SIZE, epoch)
    validation_epoch(mlp_2_640, validateloader, loss_function, BATCH_SIZE, 
                EPOCH_BATCH_SIZE, NUM_OF_EPOCHS, epoch, './results/MLP_2_640/003/ROC/',
                './results/MLP_2_640/003/confusion_matrices/')
